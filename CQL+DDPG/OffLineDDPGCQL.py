...
class DDPG:
    ...
    def optimizer(self):
        ...
        # 更新critic网络
        with torch.no_grad():
            noise=(torch.FloatTensor(a1.size()).data.normal_(0,0.2)).to(device)
        ...
        loss_critic=self.loss(y_predicted,TD_error)
        #loss_critic=torch.nn.MSELoss(y_predicted,TD_error)

        ###Calculating CQL Loss
        q1_pred_actions=self.critic.forward(s1,a1)
        q1_curr_actions=self.critic.forward(s1,self.actor.forward(s1))
        q2_curr_actions=self.critic.forward(s1,self.actor.forward(s2))
        cat_q1=torch.cat(
            [q1_pred_actions,q2_curr_actions,q1_curr_actions], 1
        )
        min_qf1_loss=torch.logsumexp(cat_q1,dim=1).mean()*alpha#logsumexp()
        min_qf1_loss=min_qf1_loss-(q1_pred_actions.mean()*alpha)
        loss_critic=(torch.tensor(0.5,dtype=torch.float32).to(device)*loss_critic)+min_qf1_loss
    ...
    def save_model(self):
        torch.save(self.target_actor.state_dict(), 'DDPG_weight/lotyoffline_CQLactor.pt')
        torch.save(self.target_critic.state_dict(), 'DDPG_weight/lotyoffline_CQLcritic.pt')
